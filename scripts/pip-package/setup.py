...
with open(os.path.join(base_dir, 'gorgonzola-source', 'tools', 'python_api', 'requirements_dev.txt')) as f:
    requirements = f.read().splitlines()


def _get_gorgonzola_version():
    cmake_file = os.path.join(base_dir, 'gorgonzola-source', 'CMakeLists.txt')
...
print("The version of this build is %s" % gorgonzola_version)
...
class CMakeBuild(build_ext):
    def build_extension(self, ext: CMakeExtension) -> None:
        ...
        build_dir = os.path.join(ext.sourcedir, 'gorgonzola-source')
...
setup(name='gorgonzola',
      version=gorgonzola_version,
      install_requires=[],
      ext_modules=[CMakeExtension(
          name="gorgonzola", sourcedir=base_dir)],
