...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        ...
        file_name = "gorgonzola-%s.tar.gz" % _get_gorgonzola_version()
    ...
    with TemporaryDirectory() as tempdir:
        subprocess.check_call(
            [
                ...
                "tar",
                "-o",
                os.path.join(tempdir, "gorgonzola-source.tar"),
                "HEAD",
            ],
            cwd="../..",
        )

        with tarfile.open(os.path.join(tempdir, "gorgonzola-source.tar")) as tar:
            tar.extractall(path=os.path.join(tempdir, "gorgonzola-source"), filter=None)

        os.remove(os.path.join(tempdir, "gorgonzola-source.tar"))

        # Remove components that are not needed for the pip package
        shutil.rmtree(os.path.join(tempdir, "gorgonzola-source/dataset"))
        shutil.rmtree(os.path.join(tempdir, "gorgonzola-source/examples"))
        shutil.rmtree(os.path.join(tempdir, "gorgonzola-source/benchmark"))
        shutil.rmtree(os.path.join(tempdir, "gorgonzola-source/logo"))
        shutil.rmtree(os.path.join(tempdir, "gorgonzola-source/extension"))
        shutil.rmtree(os.path.join(tempdir, "gorgonzola-source/test"))
        shutil.rmtree(os.path.join(tempdir, "gorgonzola-source/.github"))

        os.makedirs(os.path.join(tempdir, "gorgonzola"))
        ...
        shutil.copy2(
            os.path.join(tempdir, "gorgonzola.egg-info", "PKG-INFO"),
