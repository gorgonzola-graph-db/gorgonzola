#!/usr/bin/env python3
"""
Gorgonzola Database Migration Script

This script helps migrate Gorgonzola databases between versions.
- Sets up isolated Python environments for each Gorgonzola version
...
Usage Examples:
    ...
    python migrate-gorgonzola-db.py --old-version 0.9.0 --new-version 0.11.0 --old-db /path/to/old/database --new-db /path/to/new/database

Notes:
- Can only be used to migrate to newer Gorgonzola versions, from 0.11.0 onwards
"""
...
GORGONZOLA_FILE_EXTENSIONS = ["", ".wal", ".shadow"]


# FIXME: Replace this with a Gorgonzola query to get the mapping when available.
gorgonzola_version_mapping = {
    34: "0.7.0",
    35: "0.7.1",
    36: "0.8.2",
    37: "0.9.0",
...
minimum_gorgonzola_migration_version = "0.11.0"


def gorgonzola_version_comparison(version: str, target: str) -> bool:
    """Return True if Gorgonzola *v* is greater or equal to target version"""
...
def read_gorgonzola_storage_version(gorgonzola_db_path: str) -> int:
    """
    Reads the Gorgonzola storage version.

    :param gorgonzola_db_path: Path to the Gorgonzola database file/directory.
    :return: Storage version code as an integer.
    """
    if os.path.isdir(gorgonzola_db_path):
        gorgonzola_version_file_path = os.path.join(gorgonzola_db_path, "catalog.kz")
        if not os.path.isfile(gorgonzola_version_file_path):
            raise FileNotFoundError("Gorgonzola catalog.kz file does not exist")
    else:
        gorgonzola_version_file_path = gorgonzola_db_path

    with open(gorgonzola_version_file_path, "rb") as f:
        f.seek(4)
        # Read the next 8 bytes as a little-endian unsigned 64-bit integer
        data = f.read(8)
        if len(data) < 8:
            raise ValueError(
                f"File '{gorgonzola_version_file_path}' does not contain a storage version code."
            )
        version_code = struct.unpack("<Q", data)[0]

    if version_code in gorgonzola_version_mapping:
        return gorgonzola_version_mapping[version_code]
    else:
        raise ValueError(f"Could not map version_code {version_code} to proper Gorgonzola version.")


def ensure_env(version: str, export_dir) -> str:
    """
    Creates a venv at `{export_dir}/.gorgonzola_envs/{version}` and installs `gorgonzola=={version}`
    Returns the venv's python executable path.
    """
    # Use temp directory to create venv
    gorgonzola_envs_dir = os.path.join(export_dir, ".gorgonzola_envs")

    # venv base under the script directory
    base = os.path.join(gorgonzola_envs_dir, version)
    py_bin = os.path.join(base, "bin", "python")
    # If environment already exists clean it
    if os.path.isdir(base):
        shutil.rmtree(base)

    print(f"→ Setting up venv for Gorgonzola {version}...", file=sys.stderr)
    # Create venv
    ...
    # Install the specific Gorgonzola version
    subprocess.run([py_bin, "-m", "pip", "install", "--upgrade", "pip"], check=True)
    subprocess.run([py_bin, "-m", "pip", "install", f"gorgonzola=={version}"], check=True)
    return py_bin


def run_migration_step(python_exe: str, db_path: str, cypher: str):
    """
    Uses `python_exe` to connect to the Gorgonzola database at `db_path` and run the `cypher` query.
    """
    snippet = f"""
import gorgonzola
db = gorgonzola.Database(r"{db_path}")
...
def gorgonzola_migration(
    ...
    if not gorgonzola_version_comparison(
        version=new_version, target=minimum_gorgonzola_migration_version
    ):
        raise ValueError(
            f"New version for gorgonzola is not supported, has to be equal or higher than version: {minimum_gorgonzola_migration_version}"
        )

    print(
        f"Migrating Gorgonzola database from {old_version} to {new_version}", file=sys.stderr
    )
    print(f"Source: {old_db}", file=sys.stderr)
    print("", file=sys.stderr)

    # If version of old gorgonzola db is not provided try to determine it based on file info
    if not old_version:
        old_version = read_gorgonzola_storage_version(old_db)
    ...
    with tempfile.TemporaryDirectory() as export_dir:
        # Set up environments
        print(f"Setting up Gorgonzola {old_version} environment...", file=sys.stderr)
        old_py = ensure_env(old_version, export_dir)
        print(f"Setting up Gorgonzola {new_version} environment...", file=sys.stderr)
        new_py = ensure_env(new_version, export_dir)

        export_file = os.path.join(export_dir, "gorgonzola_export")
    ...
    # Rename new gorgonzola database to old gorgonzola database name if enabled
    if overwrite or delete_old:
        # Remove gorgonzola lock from migrated DB
        lock_file = new_db + ".lock"
        if os.path.exists(lock_file):
            os.remove(lock_file)
        rename_databases(old_db, old_version, new_db, delete_old)

    print("Gorgonzola graph database migration finished successfully!")
...
def rename_databases(old_db: str, old_version: str, new_db: str, delete_old: bool):
    ...
    if os.path.isfile(old_db):
        ...
        for ext in GORGONZOLA_FILE_EXTENSIONS:
            src = old_db + ext
            dst = backup_base + ext
        ...
        # Directory-based Gorgonzola database
...
def main():
    p = argparse.ArgumentParser(
        description="Migrate Gorgonzola DB via PyPI versions",
        epilog="""
Examples:
  %(prog)s --old-version 0.9.0 --new-version 0.11.0 \\
    --old-db /path/to/old/db --new-db /path/to/new/db --overwrite

Note: This script will create temporary virtual environments in .gorgonzola_envs/ directory
to isolate different Gorgonzola versions.
        """,
    ...
    p.add_argument(
        ...
        required=False,
        default=None,
        help="Source Gorgonzola version (e.g., 0.9.0). If not provided, automatic gorgonzola version detection will be attempted.",
    )
    p.add_argument(
        "--new-version", required=True, help="Target Gorgonzola version (e.g., 0.11.0)"
    )
