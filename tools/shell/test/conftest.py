...
from test_helper import GORGONZOLA_EXEC_PATH, GORGONZOLA_ROOT, deleteIfExists
...
class ShellTest:
    def __init__(self) -> None:
        self.shell = GORGONZOLA_EXEC_PATH
    ...
    def send_finished_statement(self, stmt: str) -> None:
        if self.shell_process:
            assert self.shell_process.expect_exact(["gorgonzola", pexpect.EOF]) == 0
...
@pytest.fixture()
def history_path():
    path = os.path.join(GORGONZOLA_ROOT, "tools", "shell", "test", "files")
...
@pytest.fixture()
def csv_path():
    return os.path.join(GORGONZOLA_ROOT, "tools", "shell", "test", "files", "vPerson.csv")


@pytest.fixture()
def init_path():
    return os.path.join(GORGONZOLA_ROOT, "tools", "shell", "test", "files", "start.cypher")
