...
def test_multi_queries_one_line(temp_db) -> None:
    ...
    test = (
        ShellTest()
        .add_argument(temp_db)
        .statement('RETURN "databases rule" AS a; RETURN "gorgonzola is cool" AS b;')
    )
    result = test.run()
    result.check_stdout("\u2502 databases rule \u2502")
...
def test_gorgonzolarc(temp_db) -> None:
    deleteIfExists(".gorgonzolarc")
    # confirm that nothing is read on startup
    test = ShellTest().add_argument(temp_db)
    result = test.run()
    result.check_not_stdout("-- Processing: .gorgonzolarc")

    # create a .gorgonzolarc file
    with open(".gorgonzolarc", "w") as f:
    ...
    result = test.run()
    deleteIfExists(".gorgonzolarc")
    result.check_stdout("-- Processing: .gorgonzolarc")
    result.check_stdout("maxRows set as 1")
    result.check_stdout("a")

    # create a .gorgonzolarc file with errors
    with open(".gorgonzolarc", "w") as f:
