...
from test_helper import GORGONZOLA_VERSION, deleteIfExists
...
def test_help(temp_db, flag) -> None:
    ...
    result = test.run()
    result.check_stdout("Gorgonzola shell")
    # with database path
...
def test_default_bp_size(temp_db, flag) -> None:
    ...
    test = ShellTest().add_argument(temp_db).add_argument(flag).add_argument("gorgonzola")
    result = test.run()
    result.check_stderr("Argument '' received invalid value type 'gorgonzola'")
...
def test_max_db_size(temp_db, flag) -> None:
    ...
    test = ShellTest().add_argument(temp_db).add_argument(flag).add_argument("gorgonzola")
    result = test.run()
    result.check_stderr("Argument 'max_db_size' received invalid value type 'gorgonzola'")
...
def test_read_only(temp_db, flag) -> None:
    ...
    test = (
        ...
        .statement('RETURN "gorgonzola is cool" AS b;')
    )
    result = test.run()
    ...
    result.check_stdout("gorgonzola is cool")
...
def test_history_path(temp_db, history_path) -> None:
    ...
    with open(os.path.join(history_path, "history.txt")) as f:
        assert f.readline() == 'RETURN "databases rule" AS a;\n'
        assert f.readline() == 'RETURN "gorgonzola is cool" AS b;\n'
...
def test_version(temp_db, flag) -> None:
    ...
    test = ShellTest().add_argument(flag)
    result = test.run()
    result.check_stdout(GORGONZOLA_VERSION)
    # with database path
    test = ShellTest().add_argument(temp_db).add_argument(flag)
    result = test.run()
    result.check_stdout(GORGONZOLA_VERSION)
...
def test_mode(temp_db, flag) -> None:
    ...
    test = (
        ShellTest()
        .add_argument(temp_db)
        .statement('RETURN "Databases Rule" AS a, "gorgonzola is cool" AS b;')
    )
    result = test.run()
    result.check_stdout("\u2502 a              \u2502 b            \u2502")
    result.check_stdout("\u2502 Databases Rule \u2502 gorgonzola is cool \u2502")
    ...
    result.check_stdout("</tr>")
    result.check_stdout("<tr>")
    result.check_stdout("<td>Databases Rule</td><td>gorgonzola is cool</td>")
    result.check_stdout("</tr>")
    result.check_stdout("</table>")
    ...
    result = test.run()
    result.check_stdout('[\n{"a":"Databases Rule","b":"gorgonzola is cool"}\n]')
    ...
    result = test.run()
    result.check_stdout('{"a":"Databases Rule","b":"gorgonzola is cool"}')
    ...
    result.check_stdout("b = gorgonzola is cool")
