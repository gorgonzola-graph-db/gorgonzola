...
TestParser::TestParser(std::string path)
    ...
    variableMap.insert({"GORGONZOLA_ROOT_DIRECTORY", Value(GORGONZOLA_ROOT_DIRECTORY)});
    variableMap.insert({"GORGONZOLA_VERSION", Value(GORGONZOLA_VERSION)});
    variableMap.insert({"GORGONZOLA_EXPORT_DB_DIRECTORY", Value(exportDBPath)});
}
...
void TestParser::genGroupName() const {
    const std::size_t subStart =
        TestHelper::appendGorgonzolaRootPath(std::string(TestHelper::E2E_TEST_FILES_DIRECTORY)).length() +
        1;
...
void TestParser::extractDataset() {
    ...
    if (datasetType == "PARQUET") {
        ...
        testGroup->dataset = currentToken.params[2];
    } else if (datasetType == "GORGONZOLA") {
        testGroup->datasetType = TestGroup::DatasetType::GORGONZOLA;
        testGroup->dataset = currentToken.params[2];
...
TestQueryResult TestParser::extractExpectedResultFromToken() {
    ...
    if (result == "ok") {
        ...
        if (line.starts_with("<FILE>:")) {
            ...
            queryResult.expectedResult.push_back(TestHelper::appendGorgonzolaRootPath(
...
TestStatement TestParser::parseStatement(const std::string& testCaseName) {
    ...
    while (true) {
        ...
        case TokenType::BATCH_STATEMENTS: {
            std::string query = paramsToString(1);
            extractConnName(query, statement);
            statement.batchStatementsCSVFile = TestHelper::appendGorgonzolaRootPath(
    ...
    while (nextLine()) {
        ...
        case TokenType::LOAD_DYNAMIC_EXTENSION: {
            ...
            loadExtensionStatement.query =
                stringFormat("LOAD EXTENSION '{}/extension/{}/build/lib{}.gorgonzola_extension'",
                    GORGONZOLA_ROOT_DIRECTORY, extensionName, extensionName);
