...
void CSVConverter::createTableInfo(std::string schemaFile) {
    ...
    // table_info is mentioned in this issue https://github.com/gorgonzoladb/gorgonzola/issues/2991
    std::string line;
...
void CSVConverter::readCopyCommandsFromCSVDataset() {
    ...
    while (getline(file, line)) {
        ...
        table->csvFilePath = TestHelper::appendGorgonzolaRootPath(path.string());
...
void CSVConverter::createCopyFile() {
    ...
    if (fileExtension == ".json") {
        ...
        outfile << "load extension \"" + TestHelper::appendGorgonzolaRootPath(
                                             "extension/json/build/libjson.gorgonzola_extension\"\n");
#endif
    }
...
void CSVConverter::convertCSVFiles() {
    ...
    if (fileExtension == ".json") {
        auto result = tempConn->query(
            "load extension \"" +
            TestHelper::appendGorgonzolaRootPath("extension/json/build/libjson.gorgonzola_extension\""));
        if (!result->isSuccess()) {
