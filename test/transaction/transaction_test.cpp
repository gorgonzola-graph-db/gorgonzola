...
TEST_F(PrivateApiTest, CloseConnectionWithActiveTransaction) {
    ...
    conn.reset();
    conn = std::make_unique<gorgonzola::main::Connection>(database.get());
    conn->query("BEGIN TRANSACTION;");
...
TEST_F(EmptyDBTransactionTest, DatabaseFilesAfterCheckpoint) {
    ...
    ASSERT_FALSE(
        std::filesystem::exists(gorgonzola::storage::StorageUtils::getTmpFilePath(databasePath)));
    ASSERT_FALSE(
        std::filesystem::exists(gorgonzola::storage::StorageUtils::getShadowFilePath(databasePath)));
    ASSERT_FALSE(
        std::filesystem::exists(gorgonzola::storage::StorageUtils::getWALFilePath(databasePath)));
    conn->query("CREATE NODE TABLE test(id INT64 PRIMARY KEY, name STRING);");
    ASSERT_TRUE(std::filesystem::exists(gorgonzola::storage::StorageUtils::getWALFilePath(databasePath)));
...
static void insertNodes(uint64_t startID, uint64_t num, gorgonzola::main::Database& database) {
    auto conn = std::make_unique<gorgonzola::main::Connection>(&database);
...
static void insertNodesWithMixedTypes(uint64_t startID, uint64_t num,
    gorgonzola::main::Database& database) {
    auto conn = std::make_unique<gorgonzola::main::Connection>(&database);
...
static void insertRelationships(uint64_t startID, uint64_t num, gorgonzola::main::Database& database) {
    auto conn = std::make_unique<gorgonzola::main::Connection>(&database);
    for (auto i = 0u; i < num; ++i) {
...
static void updateNodes(uint64_t startID, uint64_t num, gorgonzola::main::Database& database) {
    auto conn = std::make_unique<gorgonzola::main::Connection>(&database);
    for (uint64_t i = 0; i < num; ++i) {
        auto id = startID + i;
...
static void updateMixedTypeNodes(uint64_t startID, uint64_t num, gorgonzola::main::Database& database) {
    auto conn = std::make_unique<gorgonzola::main::Connection>(&database);
    for (auto i = 0u; i < num; ++i) {
        auto id = startID + i;
...
static void updateRelationships(uint64_t startID, uint64_t num, gorgonzola::main::Database& database) {
    auto conn = std::make_unique<gorgonzola::main::Connection>(&database);
    for (auto i = 0u; i < num; ++i) {
        auto fromID = startID + i;
        auto toID = (startID + i + 1) % (num * 4);
...
static void updateRelationshipsWithMixedTransactions(uint64_t startID, uint64_t num,
    bool shouldCommit, gorgonzola::main::Database& database) {
    auto conn = std::make_unique<gorgonzola::main::Connection>(&database);
    conn->query("BEGIN TRANSACTION;");
    for (auto i = 0u; i < num; ++i) {
        auto fromID = startID + i;
