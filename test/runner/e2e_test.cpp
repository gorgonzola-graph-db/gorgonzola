...
void checkGtestParams(int argc, char** argv) {
    if (argc > 1) {
        ...
        if (argument == "--gtest_list_tests") {
            ...
            scanTestFiles(TestHelper::appendGorgonzolaRootPath(TestHelper::E2E_TEST_FILES_DIRECTORY));
        }
...
int main(int argc, char** argv) {
    try {
        ...
        if (env_import_data_dir != nullptr) {
            auto path =
                TestHelper::appendGorgonzolaRootPath(std::filesystem::path(env_import_data_dir).string());
        ...
        if (argc > 1) {
            auto path = TestHelper::appendGorgonzolaRootPath(
