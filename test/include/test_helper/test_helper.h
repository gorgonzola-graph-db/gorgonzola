...
public:
    ...
    static constexpr uint64_t HASH_INDEX_MEM =
        common::HashIndexConstants::NUM_HASH_INDEXES * common::GORGONZOLA_PAGE_SIZE * 9;
    ...
    static std::string getTestListFile() {
        return appendGorgonzolaRootPath(std::string(E2E_TEST_FILES_DIRECTORY) + "/test_list");
    }

    static std::string appendGorgonzolaRootPath(const std::string& path) {
        if (std::filesystem::path(path).is_relative()) {
            return GORGONZOLA_ROOT_DIRECTORY + std::string("/") + path;
        }
        return path;
    }
    ...
    static std::filesystem::path getRootTempDir() {
        ...
        if (tempDir != nullptr) {
            return std::filesystem::path(tempDir) / "gorgonzola";
        }
        return std::filesystem::temp_directory_path() / "gorgonzola";
    }
...
#ifdef _WIN32
        // gorgonzola still doesn't support backslashes in paths on windows
