...
TEST_F(SystemConfigTest, testMaxDBSize) {
    ...
    systemConfig->maxDBSize = 2 * GORGONZOLA_PAGE_SIZE * StorageConstants::PAGE_GROUP_SIZE + 1;
    try {
        auto db = std::make_unique<Database>(databasePath, *systemConfig);
    } catch (const BufferManagerException& e) {
        ASSERT_EQ(std::string(e.what()),
    ...
    systemConfig->maxDBSize = 2 * GORGONZOLA_PAGE_SIZE * StorageConstants::PAGE_GROUP_SIZE;
    try {
        auto db = std::make_unique<Database>(databasePath, *systemConfig);
    } catch (const BufferManagerException& e) {
        ASSERT_EQ(std::string(e.what()),
    ...
    try {
        ...
        ASSERT_EQ(std::string(e.what()),
            "Buffer manager exception: The given buffer pool size should be at least " +
                std::to_string(GORGONZOLA_PAGE_SIZE) + " bytes.");
    }
