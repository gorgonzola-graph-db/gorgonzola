...
static decltype(auto) copyFlatTuple(gorgonzola::processor::FlatTuple* tuple) {
    std::vector<std::unique_ptr<gorgonzola::common::Value>> ret;
...
TEST_F(ResultValueTest, getNextExample) {
    std::unique_ptr<gorgonzola::main::QueryResult> result = conn->query("MATCH (p:person) RETURN p.*");
    std::vector<std::vector<std::unique_ptr<gorgonzola::common::Value>>> tuples;
