...
static constexpr uint64_t GORGONZOLA_PAGE_SIZE = static_cast<uint64_t>(1) << PAGE_SIZE_LOG2;
...
struct CopyConfig {
    // (#6012) Reduced from 50 * DEFAULT_VECTOR_CAPACITY to 10 * DEFAULT_VECTOR_CAPACITY
    // to reduce peak memory usage during Pandas LOAD operations. Smaller batches allow
    // internal ColumnChunk buffers to be reclaimed more frequently.
    static constexpr uint64_t PANDAS_PARTITION_COUNT = 10 * DEFAULT_VECTOR_CAPACITY;
};
