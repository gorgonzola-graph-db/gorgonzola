...
def test_ctrl_u_cleared(test) -> None:
    ...
    test.send_finished_statement('RETURN "gorgonzola is cool" AS a;\r')
    assert (
        test.shell_process.expect_exact(["\u2502 gorgonzola is cool \u2502", pexpect.EOF])
        == 0
    )
...
def test_ctrl_d_eof(test) -> None:
    ...
    assert test.shell_process.expect_exact(["gorgonzola", pexpect.EOF]) == 0
    # acts as EOF
    assert test.shell_process.expect_exact(["gorgonzola", pexpect.EOF]) == 1
