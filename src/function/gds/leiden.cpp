...
using namespace gorgonzola::algo_extension;

// Leiden method for community detection: https://en.wikipedia.org/wiki/Leiden_method.
...
// The parallel Leiden implementation here follows https://hpc.pnl.gov/people/hala/grappolo.html.
...
struct LeidenOptionalParams final : public MaxIterationOptionalParams {
    ...
    std::unique_ptr<function::OptionalParams> copy() override {
        return std::make_unique<LeidenOptionalParams>(maxIterations, maxPhases, tolerance);
    }
};
...
class ResetMicroCommVC final : public InMemParallelCompute {
public:
    explicit ResetMicroCommVC(PhaseState& state) : state{state} {}
    ~ResetMicroCommVC() override = default;

    void parallelCompute(const offset_t startOffset, const offset_t endOffset,
        const std::optional<table_id_t>&) override {
        for (auto nodeId = startOffset; nodeId < endOffset; ++nodeId) {
            state.currCommInfos.set(nodeId, CommInfo());
            state.currComm.set(nodeId, nodeId, memory_order_relaxed);
            state.nextComm.set(nodeId, nodeId, memory_order_relaxed);
            state.currCommInfos.getUnsafe(nodeId).size.store(1, memory_order_relaxed);
            const auto nodeDegree = state.nodeWeightedDegrees.get(nodeId, memory_order_relaxed);
            state.currCommInfos.getUnsafe(nodeId).degree.store(nodeDegree, memory_order_relaxed);
        }
    }

    std::unique_ptr<InMemParallelCompute> copy() override {
        return std::make_unique<ResetMicroCommVC>(state);
    }
...
public:
    explicit RunRefinementVC(PhaseState& state, const double tolerance, const double theta)
        : state{state}, tolerance{tolerance}, theta{theta} {}
    ~RunRefinementVC() override = default;
    ...
    offset_t findPotentialNewComm(const offset_t nodeId, const weight_t selfLoopWeight,
        ...
        return validMoves.back().first;
    }

    std::unique_ptr<InMemParallelCompute> copy() override {
        return std::make_unique<RunRefinementVC>(state, tolerance, theta);
    }
...
public:
    WriteResultsVC(MemoryManager* mm, GDSFuncSharedState* sharedState, FinalResults& leidenState)
        : GDSResultVertexCompute{mm, sharedState}, finalResults{leidenState} {
...
static common::offset_t tableFunc(const TableFuncInput& input, TableFuncOutput&) {
    ...
    for (auto phase = 0u; phase < config.maxPhases.getParamVal(); ++phase) {
        ...
        {
            ResetMicroCommVC resetMicro(state);
            InMemGDSUtils::runParallelCompute(resetMicro, state.graph.numNodes, input.context);
...
static std::unique_ptr<TableFuncBindData> bindFunc(main::ClientContext* context,
    ...
    columns.push_back(input->binder->createVariable(LEIDEN_ID_COLUMN_NAME, LogicalType::INT64()));
