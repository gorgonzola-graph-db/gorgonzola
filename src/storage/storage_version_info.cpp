...
storage_version_t StorageVersionInfo::getStorageVersion() {
    ...
    if (!storageVersionInfo.contains(GORGONZOLA_CMAKE_VERSION)) {
        // If the current GORGONZOLA_CMAKE_VERSION is not in the map,
        // then we must run the newest version of gorgonzola
        // LCOV_EXCL_START
    ...
    return storageVersionInfo.at(GORGONZOLA_CMAKE_VERSION);
}
