...
using gorgonzola_parquet::format::CompressionCodec;
using gorgonzola_parquet::format::ConvertedType;
using gorgonzola_parquet::format::Encoding;
using gorgonzola_parquet::format::PageType;
using gorgonzola_parquet::format::Type;
...
ColumnReader::ColumnReader(ParquetReader& reader, LogicalType type,
    ...
    uint64_t maxRepeat)
    : schema{schema}, fileIdx{fileIdx}, maxDefine{maxDefinition},
...
void ColumnReader::prepareRead(parquet_filter_t& /*filter*/) {
    ...
    defineDecoder.reset();
    block.reset();
    gorgonzola_parquet::format::PageHeader pageHdr;
...
void ColumnReader::decompressInternal(gorgonzola_parquet::format::CompressionCodec::type codec,
    ...
    case CompressionCodec::SNAPPY: {
        {
            ...
            auto res = gorgonzola_snappy::GetUncompressedLength(reinterpret_cast<const char*>(src),
        ...
        auto res = gorgonzola_snappy::RawUncompress(reinterpret_cast<const char*>(src), srcSize,
    ...
    case CompressionCodec::ZSTD: {
        auto res = gorgonzola_zstd::ZSTD_decompress(dst, dstSize, src, srcSize);
        // LCOV_EXCL_START
        if (gorgonzola_zstd::ZSTD_isError(res) || res != (size_t)dstSize) {
    ...
    case CompressionCodec::LZ4_RAW: {
        auto res = gorgonzola_lz4::LZ4_decompress_safe(reinterpret_cast<const char*>(src),
...
void ColumnReader::preparePageV2(gorgonzola_parquet::format::PageHeader& pageHdr) {
...
void ColumnReader::preparePage(gorgonzola_parquet::format::PageHeader& pageHdr) {
    auto& trans = reinterpret_cast<ThriftFileTransport&>(*protocol->getTransport());
...
void ColumnReader::prepareDataPage(gorgonzola_parquet::format::PageHeader& pageHdr) {
