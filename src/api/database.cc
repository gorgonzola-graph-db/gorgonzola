...
Database::Database(std::string_view path, const SystemConfig& config) {
    main::SystemConfig internalConfig(config.bufferPoolSize, config.maxNumThreads,
        config.enableCompression, config.readOnly, config.maxDBSize, config.autoCheckpoint,
        config.checkpointThreshold, config.forceCheckpointOnClose, config.throwOnWalReplayFailure,
...
Status Database::flush() {
    ...
    try {
        ...
        auto status = Status::FatalIOError(
            "Checkpoint failed. Database is now read-only. Please restart. Error: " +
            std::string(e.what()));
        impl_->poison(status);
        return status;
    }
