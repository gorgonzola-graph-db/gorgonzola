...
struct GORGONZOLA_API timestamp_t {
    int64_t value = 0;
...
class Timestamp {
public:
    GORGONZOLA_API static timestamp_t fromCString(const char* str, uint64_t len);

    // Convert a timestamp object to a std::string in the format "YYYY-MM-DD hh:mm:ss".
    GORGONZOLA_API static std::string toString(timestamp_t timestamp);

    // Date header is in the format: %Y%m%d.
    GORGONZOLA_API static std::string getDateHeader(const timestamp_t& timestamp);

    // Timestamp header is in the format: %Y%m%dT%H%M%SZ.
    GORGONZOLA_API static std::string getDateTimeHeader(const timestamp_t& timestamp);

    GORGONZOLA_API static date_t getDate(timestamp_t timestamp);

    GORGONZOLA_API static dtime_t getTime(timestamp_t timestamp);
    ...
    GORGONZOLA_API static timestamp_t fromEpochMicroSeconds(int64_t epochMs);

    // Create a Timestamp object from the specified epochMs.
    GORGONZOLA_API static timestamp_t fromEpochMilliSeconds(int64_t ms);

    // Create a Timestamp object from the specified epochSec.
    GORGONZOLA_API static timestamp_t fromEpochSeconds(int64_t sec);
    ...
    GORGONZOLA_API static int64_t getEpochNanoSeconds(const timestamp_t& timestamp);

    GORGONZOLA_API static int64_t getEpochMilliSeconds(const timestamp_t& timestamp);

    GORGONZOLA_API static int64_t getEpochSeconds(const timestamp_t& timestamp);

    GORGONZOLA_API static bool tryParseUTCOffset(const char* str, uint64_t& pos, uint64_t len,
    ...
    GORGONZOLA_API static timestamp_t getCurrentTimestamp();
};
