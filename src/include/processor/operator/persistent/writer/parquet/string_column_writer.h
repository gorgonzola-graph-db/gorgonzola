...
public:
    StringColumnWriterState(gorgonzola_parquet::format::RowGroup& rowGroup, uint64_t colIdx,
...
public:
    ...
    inline gorgonzola_parquet::format::Encoding::type getEncoding(
        BasicColumnWriterState& writerState) override {
        auto& state = reinterpret_cast<StringColumnWriterState&>(writerState);
        return state.isDictionaryEncoded() ? gorgonzola_parquet::format::Encoding::RLE_DICTIONARY :
                                             gorgonzola_parquet::format::Encoding::PLAIN;
    }
