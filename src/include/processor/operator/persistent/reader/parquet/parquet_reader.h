...
struct ParquetReaderScanState {
    ...
    std::unique_ptr<gorgonzola_apache::thrift::protocol::TProtocol> thriftFileProto;
...
public:
    ...
    gorgonzola_parquet::format::FileMetaData* getMetadata() const { return metadata.get(); }

private:
    std::unique_ptr<gorgonzola_apache::thrift::protocol::TProtocol> createThriftProtocol(
        common::FileInfo* fileInfo_, bool prefetch_mode) {
        return std::make_unique<
            gorgonzola_apache::thrift::protocol::TCompactProtocolT<ThriftFileTransport>>(
            std::make_shared<ThriftFileTransport>(fileInfo_, prefetch_mode));
    }
    const gorgonzola_parquet::format::RowGroup& getGroup(ParquetReaderScanState& state) {
        KU_ASSERT(
...
private:
    ...
    std::unique_ptr<gorgonzola_parquet::format::FileMetaData> metadata;
    main::ClientContext* context;
};
