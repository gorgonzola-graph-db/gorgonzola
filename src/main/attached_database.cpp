...
void AttachedDatabase::invalidateCache() {
    if (dbType != common::ATTACHED_GORGONZOLA_DB_TYPE) {
...
AttachedGorgonzolaDatabase::AttachedGorgonzolaDatabase(std::string dbPath, std::string dbName,
    ...
    if (!vfs->fileOrPathExists(path, clientContext)) {
        throw common::RuntimeException(common::stringFormat(
            "Cannot attach a remote Gorgonzola database due to invalid path: {}.", path));
    }
